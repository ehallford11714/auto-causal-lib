"""SQLite in-memory SQLAlchemy path (no network)."""
...
def sqlite_url(tmp_path):
    ...
    df = pd.DataFrame({"z": z, "treatment": treatment, "outcome": outcome, "u": rng.normal(size=n)})
    df.to_sql("obs", engine, index=False, if_exists="replace")
    engine.dispose()
    return url
...
def test_from_sqlalchemy_table(sqlite_url):
    ...
    # treatment→outcome or association involving them should appear often
...
def test_from_sqlalchemy_query(sqlite_url):
    ac = AutoCausal.from_sqlalchemy(sqlite_url, query="SELECT z, treatment, outcome FROM obs LIMIT 50")
    assert len(ac.df) == 50
